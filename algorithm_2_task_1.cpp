...
size_t approximate_maximum_matching(const Graph &graph) {
    std::vector<bool> used(graph.AsAdjencyList().size(), false);
    std::set <std::pair<size_t, size_t>> matching;
    auto edges = graph.AsAdjencyList();
...
int main(int argc, const char* argv[]) {
    ...
    const auto graph = RandomGraph(5, 0.9);
    std::ofstream graphdata("graph.txt");
    //const auto graph = StarGraph(100);
    GraphViz(graphdata, graph);
    graphdata.close();
    GraphViz(std::cout, graph);
    std::cout << approximate_maximum_matching(graph) << std::endl;
    return 0;
}